...
1. 複数解の存在を**検出**
2. 全ての解に共通する**コア条件**を抽出
3. **周辺条件**と**ユニーク項**を特定

## コア条件と周辺条件

コア条件と周辺条件の違いを理解することは、頑健なQCA解釈のために不可欠です：

| タイプ | 定義 | 解釈 |
|--------|------|------|
| **コア条件** | 全ての解に存在 | 頑健な知見；本質的な因果要因 |
| **周辺条件** | 一部の解にのみ存在 | 文脈依存的；ケースによって異なる |
...
| 成分 | 項 | 解釈 |
|------|-----|------|
| コア | `A*B` | 3つの解全てに存在；頑健な知見 |
| 周辺 | `C, D, E` | 各々1つの解に存在；文脈依存的 |
| ユニーク（M1） | `C` | 解1のみ |
| ユニーク（M2） | `D` | 解2のみ |
| ユニーク（M3） | `E` | 解3のみ |

**推奨**: コア条件（`A*B → Y`）を主要な知見として報告し、周辺条件は考察セクションで代替経路として議論してください。
